...
Project name: recur-master or recur-dev

Description: Build RECUR for Win, Mac, Linux
...
In the build step paste script:
    ...
    wget https://raw.githubusercontent.com/RecurPay/RecurPay/master/contrib/CI/jenkins.sh -O $WORKSPACE/jenkins.sh
    chmod +x $WORKSPACE/jenkins.sh
    export RELEASEDIR=/var/www/recur
...
Create directory `/var/www/recur` and make Jenkins owner of that directory

    sudo mkdir /var/www/recur
    sudo chown jenkins /var/www/recur
