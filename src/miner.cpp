...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("RECURMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("RECURMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("RECURMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("recur-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running RECURMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
